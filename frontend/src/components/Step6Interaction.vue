    ...
    <div class="main-split-layout">
      ...
      <div class="right-panel" ref="rightPanel">
        ...
        <div v-if="activeTab === 'chat'" class="chat-container">
          ...
          <div v-if="showMemoryContext && memoryItems.length > 0" class="memory-context-panel">
            <div class="mc-header">
              <span class="mc-title">{{ $t('step5.memoryContextTitle', { count: memoryItems.length }) }}</span>
              <button class="mc-close" @click="showMemoryContext = false">×</button>
            </div>
            <div class="mc-items">
              <div v-for="item in memoryItems.slice(0, 5)" :key="item.memory_id || item.id" class="mc-item">
                <div class="mc-item-header">
                  <span class="mc-item-title">{{ item.title }}</span>
                  <span class="mc-item-score">{{ ((item.importance_score || 0) * 100).toFixed(0) }}%</span>
                </div>
                <p class="mc-item-summary">{{ (item.summary || item.clean_text || '').slice(0, 120) }}</p>
                <span class="mc-item-source">{{ item.source_type || item.source }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="chat-input-area">
            <button
              ...
              @click="toggleMemoryContext"
              :title="$t('step5.memoryContextToggle')"
            >
              <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2">
...
const toggleMemoryContext = () => {
  showMemoryContext.value = !showMemoryContext.value
  if (showMemoryContext.value && chatInput.value.trim()) {
    recallMemories(chatInput.value.trim())
  }
}

const onChatInputChange = () => {
  if (!showMemoryContext.value) return
...
const recallMemories = async (query) => {
  try {
    const res = await listMemoryItems({ keyword: query, limit: 5 })
    if (res.success) {
      memoryItems.value = res.data || res.items || []
    }
  } catch {
    memoryItems.value = []
  }
...
.memory-toggle-btn:hover { color: #475569; border-color: #94A3B8; }
.memory-toggle-btn.active { color: #4338CA; border-color: #4338CA; background: #EEF2FF; }
