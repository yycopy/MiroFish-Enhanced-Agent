...
INTERVIEW_MEMORY_PREFIX = "【以下是从外部信息库中召回的相关背景记忆，供你参考】\n"
INTERVIEW_MEMORY_SUFFIX = "\n\n【基于以上背景记忆和你的自身认知，请回答以下问题】\n"


def _recall_interview_memory(prompt: str, top_k: int = 3) -> str:
    """从 MySQL/Chroma 召回与采访问题相关的历史记忆."""
    try:
        from ..services.memory_retriever import recall_memory
        items = recall_memory(prompt, top_k=top_k)
        if not items:
            return ""
        lines = [INTERVIEW_MEMORY_PREFIX]
        for item in items:
            summary = item.get("summary") or item.get("clean_text", "")
            title = item.get("title", "")
            url = item.get("url", "")
            score = item.get("importance_score", 0)
            label = title or summary[:80]
            lines.append(f"- [{score:.2f}] {label} (来源: {url})")
        lines.append(INTERVIEW_MEMORY_SUFFIX)
        return "\n".join(lines)
    except Exception:
        return ""


def _auto_start_project_ingestion(project_id: str) -> None:
    """在仿真准备完成后自动提取关键词并启动动态信息采集.
    ...
    try:
        ...
        if not keywords:
            logger.info("项目 %s 未能提取到关键词，跳过自动采集", project_id)
            return

        ProjectIngestionManager.instance().start_ingestion(
            project_id=project_id,
            keywords=keywords,
        )
        logger.info(
            "项目 %s 自动启动动态采集: keywords=%s",
            project_id, keywords,
        )
    except Exception as exc:
        logger.warning("自动启动采集失败（不阻塞推演）: %s", exc)


def optimize_interview_prompt(prompt: str) -> str:
    """
    优化Interview提问：
    1. 从 Chroma 召回相关历史记忆注入 prompt
    2. 添加前缀避免Agent调用工具
    ...
    # 召回相关外部记忆
    memory_context = _recall_interview_memory(prompt)

    if memory_context:
        return f"{INTERVIEW_PROMPT_PREFIX}{memory_context}{prompt}"
...
def start_simulation():
    ...
    try:
        ...
        enable_graph_memory_update = data.get('enable_graph_memory_update', True)  # 默认启用图谱记忆更新
        ...
        if enable_graph_memory_update:
            ...
            if not graph_id:
                logger.warning(f"未找到 graph_id，禁用图谱记忆更新: simulation_id={simulation_id}")
