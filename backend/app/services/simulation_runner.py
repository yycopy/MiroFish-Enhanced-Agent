...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        enable_graph_memory_update: bool = True,  # 默认将活动更新到Zep图谱
    ...
    def _monitor_simulation(cls, simulation_id: str, locale: str = 'zh'):
        ...
        try:
            ...
            if exit_code == 0:
                ...
                _stop_project_ingestion_for_simulation(simulation_id)
            else:
                state.runner_status = RunnerStatus.FAILED
                # 从主日志文件读取错误信息
                main_log_path = os.path.join(sim_dir, "simulation.log")
                            ...
                            if "event_type" in action_data:
                                ...
                                if event_type == "simulation_end":
                                    ...
                                    if all_completed:
                                        ...
                                        _stop_project_ingestion_for_simulation(state.simulation_id)
...
def _stop_project_ingestion_for_simulation(simulation_id: str) -> None:
    """仿真完成时自动停止该项目的动态信息采集."""
    try:
        from .simulation_manager import SimulationManager
        from .project_ingestion_manager import ProjectIngestionManager

        manager = SimulationManager()
        state = manager.get_simulation(simulation_id)
        if state and state.project_id:
            ProjectIngestionManager.instance().stop_ingestion(state.project_id)
    except Exception:
        pass
